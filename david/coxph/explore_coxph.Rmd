...
# Function to calculate the probability of event given input features and time
risk <-  function(model, newdata, time) {
  as.numeric(1-summary(survfit(model, newdata = newdata, se.fit = F, conf.int = F), times = time)$surv)
}

summary(survfit(cox_fit,
                newdata = list(rx = "A",
                               resid.ds = "yes",
                               age = 50,
                               ecog.ps = "good")),
        censored = T, times = 1000)
...
```{r}
cut_points <- unique(ovarian$futime[ovarian$fustat==1])
```
